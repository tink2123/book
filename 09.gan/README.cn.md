...
本教程源代码目录在book/gan，初次使用请您参考Book文档使用说明
...
生成对抗网络由一个生成网络与一个判别网络组成。生成网络从潜在空间（latent space）中随机采样作为输入，其输出结果需要尽量模仿训练集中的真实样本。判别网络的输入则为真实样本或生成网络的输出，其目的是将生成网络的输出从真实样本中尽可能分辨出来。而生成网络则要尽可能地欺骗判别网络。两个网络相互对抗、不断调整参数，最终目的是使判别网络无法判断生成网络的输出结果是否真实[2]。
...
本教程将 MVIST 数据集输入网络进行训练，经过10轮训练后可以看到，生成的图片已经非常接近真实图片的样子，下图中前8行是真实图片的样子，后8行是网络生成的图像效果：
<p align="center">
<img src="https://raw.githubusercontent.com/PaddlePaddle/models/develop/fluid/PaddleCV/gan/c_gan/images/DCGAN_demo.png" width="550"><br/>
图1. GAN 生成手写数字效果
</p>

将图片生成的过程以动图的形式展示出来，可以更直观的看到生成网络将一堆随机像素变成了具有意义的图片：
...
在最理想的情况下，G 可以生成与真实样本极其相似的图片G(z)，而 D 很难判断这张生成的图片究竟是不是真的，对图片的真假进行随机猜测，即 D(G(z))=0.5。但是在实际过程中，很难得到这个完美的平衡点，关于GAN的收敛理论还在持续不断的研究中。


### DCGAN

[DCGAN](https://arxiv.org/abs/1511.06434)[4] 是深层卷积网络与 GAN 的结合，其基本原理与 GAN 相同，只是将生成网络和判别网络用两个卷积网络（CNN）替代。为了提高样本的质量和收敛速度，DCGAN 在网络结构上进行了一些改进：
...
- 激活函数：在生成器（G）中，输出层使用Tanh函数，其余层采用 ReLu 函数 ; 判别器（D）中都采用LeakyReLu。
...
本次教程使用数据规模较小的 MNIST 训练生成器和判别器，关于 MNIST 的详细介绍可参考[数字识别]()。该数据集可通过paddle.dataset模块自动下载到本地。

## 训练模型

 `gan/dc_gan.py` 演示了训练的整体过程。
...
```python
gf_dim = 64 # 生成器的feature map大小
df_dim = 64 # 判别器的feature map大小
...
def G(x):
    ...
    x = deconv(x, 1, filter_size=5, padding=2, act='tanh', output_size=[28, 28],name='deconv2')
...
DCGAN采用一个随机噪声向量作为输入，输入通过与CNN类似但是相反的结构，将输入放大成二维数据。通过采用这种结构的生成模型和CNN结构的判别模型，DCGAN在图片生成上可以达到相当可观的效果。本案例中，我们利用DCGAN生成了手写数字图片，您可以尝试更换数据集生成符合个人需求的图片，或尝试修改网络结构观察生成效果。
